package red.mohist.pfworlds.Commands;

import red.mohist.pfworlds.Main;
import red.mohist.pfworlds.utilities.Config;
import red.mohist.pfworlds.utilities.itemAPI;
import java.io.File;
...
public class cmdWorld implements CommandExecutor, TabCompleter {
        ...
        for (World w : Bukkit.getWorlds()) {
            ...
            try {
                ...
                if (cfg.getConfigurationSection("worlds.") != null) {
                    ...
                    infoLore.add("§b世界边界 §8》 §7" + w.getWorldBorder().getSize());
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
        if (args.length == 1 && args[0].equalsIgnoreCase("reload") && cs.isOp()) {
            Main.instance.reloadConfig();
            Main.instance.saveConfig();
            cs.sendMessage("重载配置文件完成！");
            return true;
        }
        if (cs instanceof Player) {
            ...
            if (args.length == 2 && args[0].equalsIgnoreCase("addinfo") && cs.isOp()) {
                World w = p.getWorld();
                Config.add(w.getName(), "info", args[1]);
            ...
            if (args.length == 2 && args[0].equalsIgnoreCase("setname") && cs.isOp()) {
                ...
                Config.add(worldname, "name", args[1]);
    ...
    private List<String> params = Arrays.asList("create", "delete", "tp", "import", "unload", "info", "addinfo", "setname", "setspawn", "list", "difficulty");
